...
public class TestSeriesGeneration {
	...
	public static void main(String args[]) throws IOException,
		...
		String[] DistrictCodeArray = (new HyderabadDistrictCodeGenerator())
				.generateDistrictCode();
		ArrayList<String> Series = (new NumberSeriesGenerator())
				.generateSeriesFromDistrictCode(DistrictCodeArray);
		...
		for (String SeriesNumber : Series) {
			...
			HashSet<String> RegNumbSet = Util
					.getRegistrationNumbers(SeriesNumber);
			String Details;
			int count = 0;
			for (String RegdNum : RegNumbSet) {
				...
				if (regdNumNotInDB(RegdNum)) {
					...
					Vehicle vObj = vFetcher
							.getVehicleDetailsFromService(RegdNum);

					if (vObj != null) {
						VehicleDAO.createAndStoreEvent(vObj);
					}
					if (vObj == null) {
						IgnoreSeriesBelongingToSameDistrict(RegdNum, Series,
								IgnoreNumberSeriesSet);
						break;
					}

					/*
					 * if(vObj!= null &&
					 * vObj.getVehicleClass().contains("MCRN")&&
					 * isLuxuryCarBrand(Details)) {
					 * VListObj.addVehicleToList(vObj); }
					 * System.out.println("Vehicle : "+ (++count)
					 * +"/"+RegNumbSet.size()+ " :" +RegdNum);
					 * if(!isAWorthySeries(count,
					 * VListObj.vList.size())||vObj==null) {
					 * IgnoreSeriesBelongingToSameDistrict
					 * (RegdNum,Series,IgnoreNumberSeriesSet); break; }
					 */
				}
			}
	...
	public static boolean isLuxuryCarBrand(String Details) {
		String[] LuxuryBrandList = { "Aston", "Audi", "Bentley", "BMW",
				"Royce", "Bugatti", "Cadillac", "Jaguar", "Mercedes",
				"Ferrari", "Porsche", "Volvo", "Mitsubishi" };
	...
	public static void IgnoreSeriesBelongingToSameDistrict(String RegdNum,
			ArrayList<String> NumberSeries, HashSet<String> IgnoreNumberSeries) {
